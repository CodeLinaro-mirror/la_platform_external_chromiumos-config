...
def regen_configs(path: str, branch: str, message: str):
  ...
  # anyway.
  os.chdir(fullpath)
  subprocess.run(['repo', 'start', branch, '.'],
                 check=True,
                 capture_output=True)
...
def main():
  ...
  parser.add_argument(
      ...
      help=('Generate configs on projects matching regex. Defaults to'
  ...
  parser.add_argument(
      ...
      '--branch',
      required=True,
      help=('Name of the branch to commit config changes to. Does not need to'
  ...
  # repo doesn't like when sync is run concurrently, so run in the main thread.
  if not args.no_sync:
    print('Syncing repos: {}'.format(', '.join(repo_names)))
    subprocess.run(['repo', 'sync', '-j',
                    str(args.jobs), *repo_names],
                   check=True,
                   capture_output=True)
