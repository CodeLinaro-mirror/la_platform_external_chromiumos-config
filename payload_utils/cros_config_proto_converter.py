...
def _check_nits_value(value: float, maximum: float, description: str):
  if not 0 <= value <= maximum:
    raise Exception('Value %.1f out of range [0, %.1f] for %s' %
...
def _check_als_steps(steps: [component_pb2.Component.AlsStep],
                     max_screen_brightness_nits: float, description: str):
  ...
  if sequence_in_percent:
    ...
    _check_increasing_sequence([step.ac_backlight_nits for step in steps],
                               '%s.ac_backlight_nits' % description)
    _check_increasing_sequence([step.battery_backlight_nits for step in steps],
...
def _format_als_step(als_step: component_pb2.Component.AlsStep) -> str:
  ...
  if als_step.ac_backlight_nits:
    ac_percent = _format_power_pref_value(
        _brightness_nits_to_percent(als_step.ac_backlight_nits,
                                    als_step.max_screen_brightness))
  else:
    ac_percent = _format_power_pref_value(als_step.ac_backlight_percent)

  if (als_step.battery_backlight_nits and
      als_step.battery_backlight_nits != als_step.ac_backlight_nits):
    battery_percent = ' %s' % _format_power_pref_value(
        _brightness_nits_to_percent(als_step.battery_backlight_nits,
                                    als_step.max_screen_brightness))
  ...
  for port in ports:
    ...
    if port.HasField('index_override'):
      if port.index_override.value in overridden_indices:
        raise Exception('Duplicate port index_override %d' %
                        port.index_override.value)
      if port.index_override.value >= len(ports):
        raise Exception('Port index_override %d outside range [0, %d)' %
  ...
  if light_sensor.lid_lightsensor == present:
    ...
    if hw_features.screen.panel_properties.no_als_ac_brightness:
      ...
      brightness_pct_calc = _brightness_nits_to_percent(
          ...
          hw_features.screen.panel_properties.max_screen_brightness)
      result['internal-backlight-no-als-ac-brightness'] = brightness_pct_calc
...
def _build_derived_power_prefs(config: Config) -> dict:
  ...
  if len(hw_features.usb_c.ports) > 1:
    ...
    result['charging-ports'] = _build_charging_ports(hw_features.usb_c.ports)
...
def _build_health_routines(health_config):
  ...
  if routines.HasField('battery_health'):
    ...
    _upsert(routines.battery_health.percent_battery_wear_allowed,
            battery_health_result, 'percent-battery-wear-allowed')
...
def _get_arc_camera_features(camera, camera_config):
  ...
  features = [
      ...
      _feature('android.hardware.camera.any', count > 0 or
               camera_config.has_external_camera),
...
def _generate_arc_media_profiles(hw_features, sw_config, program, dtd_path):
  ...
  if (not camera_config.generate_media_profiles and
      not program.generate_camera_media_profiles):
    return None
