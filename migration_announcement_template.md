**Fill in all sections demarcated in <> brackets and then delete this. Send to
g/chromeos-<program>, g/chromeos-peng-sies, platform project leads, and
g/cros-boxster-announce.**

**Configuration for <program> will now be managed on a per-project basis
with the goal of enabling autonomous and flexible partner contributions and
clearer API boundaries. Once we receive acknowledgement from project owners, the
migration CL will be submitted within 2 to 3 business days**

Starlark -- a Python-like configuration-management language -- will be used to
generate Protocol Buffer configuration payloads in <repos> and model.yaml will
be deleted from <private overlay>. A few resources
to get started:
...
*   <new config repos>: Contains Starlark files, generated protobuf payloads,
    and backwards-compatible JSON payloads.
*   <private overlay>:  <chromeos-config-bsp ebuild> sets `CROS_WORKON_PROJECT`
    to <new config repos>. model.yaml is deleted.
