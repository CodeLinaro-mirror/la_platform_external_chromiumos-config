...
def attributes_at_ref(ref: str,
                      id_only=False) -> dut_attribute_pb2.DutAttributeList:
  ...
  proc = subprocess.run(
      ['git', 'cat-file', 'blob', f'{ref}:generated/dut_attributes.jsonproto'],
      check=True,
      capture_output=True,
  )
...
def check_attributes_removed():
  ...
  if removed_attr_ids:
    print(
        "DutAttributes cannot be removed because branched code may depend on them.\n\n"
...
def check_valid_attributes():
  ...
  def validate_attr_fields(attr):
    ...
    for field_spec in fields:
      field_infos = proto_utils.resolve_field_path(root_msg, field_spec.path)
      ...
      for ii, (field, info) in enumerate(zip(fields, field_infos)):
        ...
        if not info:
          ...
          errors.append(
              format_error(
                  attr,
                  "'{}' -- field '{}' does not exist in message '{}'".format(
                      field_spec.path, field, msg_name)))
          break
        ...
        if info.repeated:
          errors.append(
              format_error(
                  attr,
                  "'{}' -- field '{}' is repeated, must be singular".format(
                      field_spec.path, field)))
      ...
      if terminal_info:
        if terminal_info.typeid in NON_VALUE_TYPES:
          errors.append(
              format_error(
                  attr, "'{}' -- last field '{}' must be value type".format(
                      field_spec.path, terminal_info.name)))

        # and that it's not a floating point value
        if terminal_info.typeid in FLOAT_TYPES:
          errors.append(
              format_error(
                  attr, "'{}' -- floating point fields are not allowed".format(
                      field_spec.path)))
  ...
  def validate_attr(attr):
    """Validate the properties of an attribute itself, return list of errors."""
    attr_map = {}
    ...
    def maybe_add_label(attr, label):
      ...
      if label in attr_map:
        return [
            format_error(
                attr, "label '{}' already exists in attribute '{}'".format(
